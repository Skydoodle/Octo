...
export function daysUntil(dateStr: string): number {
  return calendarDaysBetween(dateOnlyFromLocalDate(today()), dateStr) ?? Number.POSITIVE_INFINITY
}
...
export function getOverdueCount(beyannameler: Beyanname[]): number {
  return beyannameler.filter(b =>
    b.status !== 'odendi' && daysUntil(b.sonTarih) < 0
  ).length
}
...
export function computeDeadline(type: string, donem: string): string {
  ...
  const adjustWeekend = (value: string): string => {
    const ms = dateOnlyToEpochMs(value)
    if (ms === null) return value
    const weekday = new Date(ms).getUTCDay()
    if (weekday === 6) return addDateOnlyDays(value, 2) ?? value
    if (weekday === 0) return addDateOnlyDays(value, 1) ?? value
    return value
  }
  ...
  if (/^\d{4}-Q[1-4]$/.test(donem)) {
    ...
    return adjustWeekend(`${year}-${pad(month)}-17`)
  }

  if (/^\d{4}$/.test(donem)) {
    // kurumlar: end of April following year
    return adjustWeekend(`${parseInt(donem) + 1}-04-30`)
  }
  ...
  return adjustWeekend(`${ny}-${pad(nm)}-${day}`)
}
