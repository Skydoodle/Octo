...
describe('Sales Workbench V1 interface', () => {
  ...
  it('keeps accountants read-only and hides preparation action', () => { expect(pageSource).toContain('Salt okunur görünüm');expect(pageSource).toContain('canWrite&&'); expect(pageSource).not.toMatch(/createSales|updateSales|transitionSales|refreshCompany/) })
